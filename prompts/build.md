...
- **If the item outgrows the iteration, split it instead of grinding.** When the item is still incomplete after roughly 60 turns of work, or accumulated output is crowding your context: bring what you have built to green, create a task file for the remainder (next unused `NNN`) and insert its index entry **directly after the current item's entry** (splitting is the one case where inserting beats appending — the remainder keeps the original's priority), tiering it for what the remaining work actually needs — an item that outgrew its iteration is usually `(heavy)` — then mark the current item done with a completion note naming the split-out remainder, and proceed to Phase 4 as normal. A fresh iteration on the remainder is faster and cheaper than finishing at the context ceiling.
...
1. Close out the item in both places:
   - **Its task file** (`plan/NNN-*.md`) — set `**Status:**` to `Done` and fill in the **Completion notes** section: what shipped, test counts, and any deviation with its tracking reference (e.g. an `OPEN_QUESTIONS.md` id). **Keep it to 3 lines** so a later targeted read stays cheap; the narrative, evidence and reasoning belong in `PROGRESS.md` (step 2). If the item proved materially harder than its tier suggested — it needed an Opus subagent to get unstuck, or it had to be split — say so in one clause, so re-planning can re-tier what remains.
   - **`IMPLEMENTATION_PLAN.md`** — flip the entry to `- [x]` and leave the rest of the line exactly as it is, tier included. **No completion notes in the index**: it is a queue every future iteration re-reads in full, so it stays one line per item.
   - **Never delete an entry or its task file.** The plan is an append-only ledger: completed items stay as a record of what shipped. You may **append** new items — a new task file plus its index entry, tiered `(light)` or `(heavy)` — if this iteration surfaced follow-up work, but do not remove or rewrite existing ones.
...
- For bugs you notice outside the current item, document them as new items — a task file under `plan/` plus its index entry, tiered `(light)` or `(heavy)` — instead of fixing them inline; a future iteration will pick them up.
- **Every index entry you write carries a tier.** `(light)` for work that follows an established pattern with clear specs and localised changes; `(heavy)` for cross-cutting changes, spec reconciliation, or debugging of unknown cause. Default to `(heavy)` when unsure — an untiered entry falls back to the run's default model, so the loop keeps working but stops saving anything.

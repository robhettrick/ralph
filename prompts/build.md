...
3. Select the next `- [ ]` entry and implement that instead. A blocked item is the one case where the iteration moves on to another item, because nothing was built for this one — the one-item rule bounds what you *build*, not how many blocked entries you skip past. If every remaining entry is blocked, report `no open items` and stop.
...
- For bugs you notice outside the current item, document them as new items — a task file under `plan/` plus its index entry, tiered `(light)` or `(heavy)` — instead of fixing them inline; a future iteration will pick them up. A test failing right now is the exception: Phase 3 says fix it in this increment.
